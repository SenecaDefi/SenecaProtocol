...
contract SenecaOracleEUR is IOracle {

    string public oraclename = 'EUR Oracle';
    IAggregator public iaggregator;
    address public aggregator;
    string public key;

    AggregatorV3Interface public arbFeed;
    ...
    constructor(address oracle, string memory _key, address _sequencerUptimeFeed) {
        iaggregator = IAggregator(oracle);
        aggregator = oracle;
        key = _key;

        arbFeed = AggregatorV3Interface(
            aggregator
        );
    ...
    function _get() internal view returns (uint256) {
        ...
        uint256 fullAnswer = uint256(getEthPrice());
